...
function App() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://192.168.8.107:3000/users');
    ...
    const handleAdd = async () => {
        if (formData.age === '' || formData.first_name === '' || formData.last_name === '' || formData.phone_no === '') {
            window.alert('Please fill in all fields');
        } else {
            try {
                await axios.post('http://192.168.8.107:3000/users', formData);
                fetchData(); 
                setFormData({
                    first_name: '',
                    last_name: '',
    ...
    const handleSaveEdit = async () => {
        if (selectedRowIndex !== null) {
            try {
                await axios.put(`http://192.168.8.107:3000/users/${rows[selectedRowIndex].student_no}`, formData);
                fetchData(); 
                setFormData({
    ...
    const handleDelete = async (index) => {
        ...
        if (shouldDelete) {
            try {
                await axios.delete(`http://192.168.8.107:3000/users/${rows[index].student_no}`);
                fetchData(); 
            } catch (error) {
    ...
    return (
        <div className="mx-auto flex justify-center h-screen items-start">
            <table className="table-auto">
                <thead>
                    <tr>
                        <th className="px-8 py-5 w-auto"><input className='h-10 text-center border rounded w-auto' type="text" placeholder="First Name" name="first_name" value={formData.first_name} onChange={handleChange} required /></th>
                        <th className="px-8 py-5 w-auto"><input className='h-10 text-center border rounded w-auto' type="text" placeholder="Last Name" name="last_name" value={formData.last_name} onChange={handleChange} required /></th>
                ...
                <tbody>
                    {rows.map((row, index) => (
                        <tr key={index}>
                            <td className="border px-4 py-2 w-auto">{row.first_name}</td>
                            <td className="border px-4 py-2 w-auto">{row.last_name}</td>
