...
function App() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://192.168.8.107:3000/students');
    ...
    const handleAdd = async () => {
        if (formData.age === '' || formData.firstname === '' || formData.lastname === '' || formData.phone_no === '') {
            ...
            try {
                await axios.post('http://192.168.8.107:3000/students', formData);
                fetchData(); 
                setFormData({
                    firstname: '',
                    lastname: '',
    ...
    const handleSaveEdit = async () => {
        if (selectedRowIndex !== null) {
            try {
                await axios.put(`http://192.168.8.107:3000/students/${rows[selectedRowIndex]._id}`, formData);
                fetchData(); 
                setFormData({
    ...
    const handleDelete = async (index) => {
        ...
        if (shouldDelete) {
            try {
                await axios.delete(`http://192.168.8.107:3000/students/${rows[index]._id}`);
                fetchData(); 
            } catch (error) {
